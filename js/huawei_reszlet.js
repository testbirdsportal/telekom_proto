...
function getAlsoAr(also){
	var b =  [1970, 1970, 2610, 3410, 3470, 3840,5310, 5340];
...
function move_tooltip2(point,bool = true){
	if(point == 0){
		...
		maradek /= 7;
		maradek *= point;
		$('.tooltip_arrow2').css('left',point*(tooltip/7)+point*2.5+'px');
		$('.own_tooltip2').css('left',maradek+'px');
		$('.marker2').css('left',parseInt(left)-10+'px');
	}

	var b = 0;
	if(point == 2 || point == 3 || point == 4){
	   b = 1;
	}
...
function last_tooltip(){
		...
		var maradek = width-tooltip-5;
		//$('.own_tooltip2').css('left',maradek+'px');
		$('.own_tooltip2').css('left','27%');
		$('.tooltip_arrow2').css('left','73%');
		var left = $('#slider_holder2 .slider_point_5').css('margin-left');
...
function change_ar(){
	$('input[name="ar"]').change(function(){
		...
		if(ertek == 'egy_osszegben'){
			window.location.replace("huawei_egyben.html");
